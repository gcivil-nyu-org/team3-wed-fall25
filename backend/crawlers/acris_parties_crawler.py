...
class AcrisPartiesCrawler(DataCrawler):
    ...
    def _headers(self):
        return {"Accept": "application/json", "User-Agent": "AcrisParties/1.0"}
    ...
    def _req(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i in range(1, self.max_retries + 1):
            ...
            if r.status_code in (429, 500, 502, 503, 504):
                print(
                    f"[AcrisParties] attempt {i} {r.status_code} retry {backoff:.1f}s"
                )
                time.sleep(backoff)
                backoff *= 2
                continue
    ...
    def _discover(self) -> Set[str]:
        ...
        print(f"[AcrisParties] fields: {sorted(self._avail)}")
        return self._avail
    ...
    def _resolve(self) -> Dict[str, Optional[str]]:
        ...
        print(f"[AcrisParties] map: {self._res}")
        return self._res
    ...
    def fetch(self, limit: int = 5000, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[AcrisParties] Fetch failed: {e}")
            return []
        res = self._resolve()
    ...
    def load(self, rows: List[Dict[str, Any]]) -> None:
        ...
        with PostgresClient() as db:
            try:
                ...
                print(f"[AcrisParties] Insert failed: {e}")
                raise
