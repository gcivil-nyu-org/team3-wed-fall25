...
class AcrisMasterCrawler(DataCrawler):
    ...
    def _headers(self):
        return {"Accept": "application/json", "User-Agent": "AcrisMaster/1.0"}
    ...
    def _req(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i in range(1, self.max_retries + 1):
            ...
            if r.status_code in (429, 500, 502, 503, 504):
                print(f"[AcrisMaster] attempt {i} {r.status_code} retry {backoff:.1f}s")
                time.sleep(backoff)
                backoff *= 2
                continue
    ...
    def _discover(self) -> Set[str]:
        ...
        print(f"[AcrisMaster] fields: {sorted(self._avail)}")
        return self._avail
    ...
    def _resolve(self) -> Dict[str, Optional[str]]:
        ...
        self._res = res
        print(f"[AcrisMaster] map: {res}")
        return res

    def fetch(
        self, limit: int = 5000, offset: int = 0, order: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        avail = self._discover()
        ...
        try:
            ...
            print(f"[AcrisMaster] Fetch failed: {e}")
            return []
        res = self._resolve()

        def g(n):
            return res.get(n) if res.get(n) in avail else None
    ...
    def load(self, rows: List[Dict[str, Any]]) -> None:
        ...
        with PostgresClient() as db:
            try:
                ...
                print(f"[AcrisMaster] Insert failed: {e}")
                raise
