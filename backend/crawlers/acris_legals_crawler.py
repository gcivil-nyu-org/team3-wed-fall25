...
class AcrisLegalsCrawler(DataCrawler):
    ...
    def _headers(self):
        return {"Accept": "application/json", "User-Agent": "AcrisLegals/1.0"}
    ...
    def _req(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i in range(1, self.max_retries + 1):
            ...
            if r.status_code in (429, 500, 502, 503, 504):
                print(f"[AcrisLegals] attempt {i} {r.status_code} retry {backoff:.1f}s")
                time.sleep(backoff)
                backoff *= 2
                continue
    ...
    def _discover(self) -> Set[str]:
        if self._avail is not None:
            return self._avail
        d = self._req({"$limit": 1})
        self._avail = set(d[0].keys()) if d else set()
        print(f"[AcrisLegals] fields: {sorted(self._avail)}")
        return self._avail
    ...
    def _resolve(self) -> Dict[str, Optional[str]]:
        ...
        a = self._discover()
        self._res = {
            k: next((c for c in v if c in a), None)
            for k, v in self.FIELD_CANDIDATES.items()
        }
        print(f"[AcrisLegals] map: {self._res}")
        return self._res
    ...
    def fetch(self, limit: int = 5000, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[AcrisLegals] Fetch failed: {e}")
            return []
        res = self._resolve()
        g = lambda n: (res.get(n) if res.get(n) in a else None)
        out = []
        for d in data:
            doc_id = d.get(g("document_id"))
            if not doc_id:
                continue
            borough = self._to_int(d.get(g("borough")))
            block = self._to_int(d.get(g("block")))
            lot = self._to_int(d.get(g("lot")))
            out.append(
                {
                    "document_id": doc_id,
                    "borough": borough,
    ...
    def load(self, rows: List[Dict[str, Any]]) -> None:
        ...
        with PostgresClient() as db:
            try:
                ...
                print(f"[AcrisLegals] Insert failed: {e}")
                raise
