...
class RegistrationContactCrawler(DataCrawler):
    ...
    DEFAULT_SELECT_CANDIDATES = [
        ...
        "business_street_name", "businessstreetname",
        "business_city", "businesscity",
        "business_state", "businessstate",
        "business_zip", "businesszip",
        "business_apartment", "businessapartment",
    ]
    ...
    def _headers(self) -> Dict[str, str]:
        return {"Accept": "application/json", "User-Agent": "RegistrationContactCrawler/1.0"}
    ...
    def _request(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if resp.status_code in (429, 500, 502, 503, 504):
                print(f"[RegistrationContactCrawler] attempt {attempt} -> {resp.status_code}, retry in {backoff:.1f}s")
                time.sleep(backoff)
                backoff *= 2
                continue
    ...
    def _build_params(
        ...
        *,
        ...
        if registration_id is not None and resolved.get("registration_id"):
            where_clauses.append(f"{resolved['registration_id']} = {int(registration_id)}")
        ...
        if order:
            ...
            order_col = resolved.get("registration_contact_id") or (
                "registrationcontactid" if "registrationcontactid" in available else None
            )
            if order_col:
        ...
        *,
        ...
        try:
            params = self._build_params(
                ...
                registration_id=registration_id, contact_type=contact_type,
                state=state, city=city, zip_code=zip_code,
                name_like=name_like, extra=extra,
            )
        ...
        for d in data:
            row = {
                "registration_contact_id": self._to_int(d.get(avail(resolved.get("registration_contact_id")))),
                "registration_id": self._to_int(d.get(avail(resolved.get("registration_id")))),
                "type": d.get(avail(resolved.get("type"))),
                "contact_description": d.get(avail(resolved.get("contact_description"))),
                ...
                "business_house_number": d.get(avail(resolved.get("business_house_number"))),
                "business_street_name": d.get(avail(resolved.get("business_street_name"))),
