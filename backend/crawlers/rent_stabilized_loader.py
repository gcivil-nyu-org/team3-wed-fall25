...
class RentStabilizedLoader(DataCrawler):
    ...
    def __init__(self, data_file_path: str = None):
        ...
        if data_file_path is None:
            ...
            self.data_file_path = (
                project_root / "data" / "rent_stabilized_all_boroughs.json"
            )
        else:
    ...
    def load(self, rows: List[Dict[str, Any]]) -> None:
        ...
        if len(deduplicated_rows) != len(rows):
            print(
                f"[{self.__class__.__name__}] Removed {len(rows) - len(deduplicated_rows)} duplicate (year, bbl) combinations from batch"
            )
    ...
    def load_all(self, batch_size: int = 1000) -> int:
        ...
        """
        print(
            f"[RentStabilizedLoader] Starting to load all data from {self.data_file_path}"
        )
        ...
        while True:
            ...
            if not batch:
                print("[RentStabilizedLoader] No more data. Stopping.")
                break
        ...
        print(
            f"[RentStabilizedLoader] Completed. Total loaded: {total_loaded} records."
        )
        return total_loaded
