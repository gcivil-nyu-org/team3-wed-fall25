...
class RegistrationCrawler(DataCrawler):
    ...
    def fetch(self, limit: int = 1000, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for d in data:
                bbl = d.get("bbl") or compute_bbl(
                    d.get("boroid"), d.get("block"), d.get("lot")
                )
                if not bbl:
                    skipped += 1
                    continue
                ...
                mapped.append(
                    {
                        ...
                        "zip": d.get("zip"),
                        "community_board": (
                            int(d.get("communityboard"))
                            if d.get("communityboard")
                            else None
                        ),
                        "last_registration_date": d.get("lastregistrationdate"),
                        "registration_end_date": d.get("registrationenddate"),
                        "registration_id": (
                            int(d.get("registrationid"))
                            if d.get("registrationid")
                            else None
                        ),
                        "building_id": (
                            int(d.get("buildingid")) if d.get("buildingid") else None
                        ),
                    }
                )

            print(
                f"[RegistrationCrawler] Fetched {len(mapped)} rows (skipped {skipped} without BBL)."
            )
            return mapped
        ...
        with PostgresClient() as db:
            try:
                count = db.bulk_insert(
                    self.TABLE_NAME, self.COLUMNS, rows, conflict_target=["bbl"]
                )
                print(
                    f"[RegistrationCrawler] Inserted {count} rows into {self.TABLE_NAME}."
                )
