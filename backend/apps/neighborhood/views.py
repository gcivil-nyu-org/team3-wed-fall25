...
class HeatmapDataView(APIView):
    ...
    def get(self, request):
        ...
        time_range = request.query_params.get("time_range", "all")  # "all", "6months", "1year", "3years"
        ...
        try:
            ...
            heatmap_data = repo.get_heatmap_data(
                ...
                borough=borough,
                limit=limit,
                time_range=time_range if time_range in ["all", "6months", "1year", "3years"] else "all",
            )
...
class AffordableHousingBBLsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response(
                {"detail": f"Internal error while fetching affordable housing BBLs: {e}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class FilteredViolationsView(APIView):
    """
    GET /api/neighborhood/filtered-violations?min_lat=40.7&max_lat=40.8&min_lng=-74.0&max_lng=-73.9&min_open_violations=5&max_open_violations=50
    
    Get violation points with advanced filtering options:
    - Status filters: min_open_violations, max_open_violations, min_closed_violations, max_closed_violations
    ...
    def get(self, request):
        ...
        try:
            ...
            min_open_violations = int(min_open_violations) if min_open_violations else None
            max_open_violations = int(max_open_violations) if max_open_violations else None
            min_closed_violations = int(min_closed_violations) if min_closed_violations else None
            max_closed_violations = int(max_closed_violations) if max_closed_violations else None
