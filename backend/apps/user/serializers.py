...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'tenant_type', 'phone_number', 'landlord_type', 'organization_name',
            'hpd_registration_number', 'business_phone', 'first_name', 'last_name'
        )
        extra_kwargs = {
        ...
        if role == 'tenant':
            ...
            if landlord_type in ['property_management', 'corporate_landlord']:
                if not attrs.get('organization_name'):
                    raise serializers.ValidationError("Organization name is required for this landlord type")
...
class UserSerializer(serializers.ModelSerializer):
    ...
    landlord_type_display = serializers.CharField(source='get_landlord_type_display', read_only=True)
    
    class Meta:
        model = CustomUser
        fields = (
            'id', 'username', 'email', 'first_name', 'last_name', 'role',
            'role_display', 'is_verified', 'tenant_type', 'tenant_type_display',
            'phone_number', 'landlord_type', 'landlord_type_display',
            'organization_name', 'hpd_registration_number', 'business_phone',
