...
class CustomUser(AbstractUser):
    ...
    def clean(self):
        ...
        if (self.role == 'landlord' and 
            self.landlord_type in ['property_management', 'corporate_landlord'] and 
            not self.organization_name):
            raise ValidationError('Organization name is required for this landlord type')
    
    def send_verification_email(self):
        """Send email verification to user"""
        verification_url = f"{settings.FRONTEND_URL}/verify-email?token={self.verification_token}"
