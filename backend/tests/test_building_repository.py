...
from infrastructures.postgres.building_repository import (  # 경로는 프로젝트 구조에 맞게 조정
    BuildingRepository,
)
...
def main():
    ...
    if building.registration:
        print(
            f"Address: {building.registration.house_number} {building.registration.street_name}, ZIP={building.registration.zip}"
        )
    if building.rent_stabilized:
        print(
            f"Rent-Stabilized: {building.rent_stabilized.status} (year={building.rent_stabilized.source_year})"
        )
    ...
    print(
        f"ACRIS docs: {len(building.acris_master)} "
    ...
    print(
        json.dumps(
            asdict(building), indent=2, default=default_serializer, ensure_ascii=False
        )
    )
