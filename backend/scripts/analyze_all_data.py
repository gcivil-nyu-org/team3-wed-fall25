...
def analyze():
    with PostgresClient() as db:
        ...
        eviction_bbls_with_loc = db.scalar(
            "SELECT COUNT(DISTINCT bbl) FROM building_evictions WHERE latitude IS NOT NULL AND longitude IS NOT NULL"
        )
        violation_bbls = db.scalar(
            "SELECT COUNT(DISTINCT bbl) FROM building_violations"
        )
        complaint_bbls = db.scalar(
            "SELECT COUNT(DISTINCT bbl) FROM building_complaints"
        )

        print(f"  building_registrations: {reg_bbls:,} unique BBLs")
        print(f"  building_evictions (all): {eviction_bbls:,} unique BBLs")
        print(
            f"  building_evictions (with location): {eviction_bbls_with_loc:,} unique BBLs"
        )
        ...
        print(
            f"  Violations that CAN use eviction location (same BBL): {violations_with_shared_loc:,}"
        )
        print(
            f"  Complaints that CAN use eviction location (same BBL): {complaints_with_shared_loc:,}"
        )
        ...
        print(
            f"  Total buildings we CAN map (have location + have data): {total_mappable:,}"
        )
