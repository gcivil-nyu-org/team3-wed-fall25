...
GEOAPIFY_API_KEY = os.environ.get(
    "GEOAPIFY_API_KEY", ""
)  # Optional - works without key but slower
...
def geocode_with_geoapify(
    house_number: str, street_name: str, borough: str, zip_code: Optional[str] = None
...
def geocode_with_nominatim(
    ...
    try:
        ...
        response = requests.get(
            NOMINATIM_API, params=params, headers=headers, timeout=10
        )
        response.raise_for_status()
        data = response.json()
...
def geocode_single_building(
    building: dict,
) -> Tuple[str, bool, Optional[Tuple[float, float]]]:
...
def geocode_buildings(
    batch_size: int = 100, max_buildings: Optional[int] = None, max_workers: int = 10
):
    """
    ...
    with PostgresClient() as db:
        ...
        print(
            f"Processing in batches of {batch_size} with {max_workers} parallel workers..."
        )
        print("Using Geoapify (5 req/sec) + Nominatim (1 req/sec fallback)\n")
        ...
        for batch_start in range(0, total, batch_size):
                ...
                for bbl, future in futures:
                    try:
                        ...
                        if success:
                            if coords:
                                print(
                                    f"  [{processed}/{total}] BBL {result_bbl}: ✓ ({coords[0]:.6f}, {coords[1]:.6f})"
                                )
                            else:
                                print(
                                    f"  [{processed}/{total}] BBL {result_bbl}: ✓ (already geocoded)"
                                )
                            success_count += 1
                        else:
                            print(
                                f"  [{processed}/{total}] BBL {result_bbl}: ✗ (failed)"
                            )
                            fail_count += 1
            ...
            print(
                f"  Batch complete: {success_count} succeeded, {fail_count} failed so far"
            )
        ...
        final_with_location = db.scalar(
            "SELECT COUNT(*) FROM building_locations WHERE has_location = TRUE"
        )
        final_without_location = db.scalar(
        ...
        print(
            f"  - Coverage: {(final_with_location/(final_with_location+final_without_location)*100):.1f}%"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Geocode buildings without location data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum buildings to geocode (default: all)",
    )
    parser.add_argument(
    ...
    try:
        geocode_buildings(
            batch_size=args.batch_size, max_buildings=args.max, max_workers=args.workers
        )
